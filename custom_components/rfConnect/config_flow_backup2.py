...
class RFConnectConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_learn_codes(
        ...
        """Start learning RF codes for the device."""
        if user_input is not None:
            # Check if user wants manual input
            if user_input.get("manual_input"):
                return await self.async_step_manual_input()
            
            # Start automatic learning - go directly to listening
            return await self.async_step_listen_code()

        # Simple form with just submit button and manual input checkbox
        code_count = len(self._rf_codes) // 2  # Divide by 2 since we add ON/OFF pairs
        ...
        if user_input is not None:
            ...
            if device_id and channel is not None and state_type:
                try:
                    ...
                    # Go to codes_learned screen
                    return await self.async_step_codes_learned({"code_received": True})
